...
#define BIMODAL_TABLE_SIZE 196608
#define MAX_INDEX_BITS 14
#define TAGE_TABLES 2
...
uint8_t Tage::predict(uint64_t ip){
    ...
    else
    {
        ...
        STRONG = abs(2*T[pred_comp-1][index].counter + 1 - (1 << TAGE_CONTER_BITS)) > 1;
        ...
        { 
            tage_pred = alt_pred;
            // std::cout << "pred_comp" << int(pred_comp) << std::endl;
            // std::cout << "alt_comp" << int(alt_comp) << std::endl;
            debug_ct[2]++;
        }
    }
    // for (int i = 0; i < BIMODAL_TABLE_SIZE; i++)
    //     std::cout << int(T0[i]) << std::endl;
