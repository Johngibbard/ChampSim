...
def merge_json(predictors):
    ...
    for i in predictors:
        ...
        for j in range(0,len(merge_list)):
            print(LOG_PATH+merge_list[j])
            with open(LOG_PATH+merge_list[j],"r") as file2:
                    data2 = json.load(file2)
                    data.append(data2[0])
                    file2.close()
        with open(LOG_PATH+i+".json","w+") as file1:
            try: 
